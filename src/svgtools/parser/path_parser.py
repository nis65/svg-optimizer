...
from svgtools.geometry import Geometry, Path, Point
...
def parse_path_string(text: str | None) -> Geometry:  # noqa: PLR0912
    ...
    while True:
        ...
        match command:
            case "m" | "M":
                current_state, parsed_elements = _parse_mM_list(
                    command, current_state, token_iterator
                )
            case "l" | "L":
                current_state, parsed_elements = _parse_any_list(
                    command,
                    current_state,
                    token_iterator,
                    LineTo.parameter_counts[command],
                    _parse_lL,
                )
            case "h" | "H":
                current_state, parsed_elements = _parse_any_list(
            ...
            case "z" | "Z":
                current_state, parsed_elements = _parse_any_list(
                    ...
                    current_state,
                    token_iterator,
                    ClosePath.parameter_counts[command],
                    _parse_zZ,
                )
            case "q" | "Q":
                current_state, parsed_elements = _parse_any_list(
                    ...
                    current_state,
                    token_iterator,
                    QuadraticBezier.parameter_counts[command],
                    _parse_qQ,
                )
            case "t" | "T":
                current_state, parsed_elements = _parse_any_list(
            ...
            case "c" | "C":
                current_state, parsed_elements = _parse_any_list(
                    ...
                    current_state,
                    token_iterator,
                    CubicBezier.parameter_counts[command],
                    _parse_cC,
                )
            case "s" | "S":
                current_state, parsed_elements = _parse_any_list(
