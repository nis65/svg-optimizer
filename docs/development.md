...
* If there are `ruff check` errors, inspect them. If they can be fixed automatically, use `ruff check --preview --extend-select PLR --output-format=full src --fix`. If not, fix manually.
