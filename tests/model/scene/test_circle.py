...
def test_circle_construction():
    c = Circle(geometry=(GeometryCircle(GeometryPoint(1.5, -2.0), 2)))
    assert c.geometry.center.x == 1.5
    assert c.geometry.center.y == -2.0
    assert c.geometry.radius == 2

def test_circles_are_equal():
    assert ( Circle(geometry=(GeometryCircle(GeometryPoint(1.5, -2.0), 2))) ==
             Circle(geometry=(GeometryCircle(GeometryPoint(1.5, -2.0), 2)))
           )
