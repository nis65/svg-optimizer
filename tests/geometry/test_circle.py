...
def test_circles_are_equal():
    assert Circle(center=Point(1, 2), radius=3) == Circle(center=Point(1, 2), radius=3)
...
def test_circle_transformed_bounding_box():
    ...
    assert Point.points_are_close(
        expected, c.points_for_bounding_box(4), abs_tol=GEOMETRY_ABS_TOL
    )
    sqrt2 = math.sqrt(2)
    expected = {
        Point(0, 2),
    ...
    assert Point.points_are_close(
        expected, c.points_for_bounding_box(8), abs_tol=GEOMETRY_ABS_TOL
    )
